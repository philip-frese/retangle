...
program
  ...
  .action(async (opts) => {
    const fileConfig = await loadConfig(opts.project).catch(() => ({} as Partial<RetangleConfig>));

    const config: RetangleConfig = {
      projectPath: opts.project ?? fileConfig.projectPath ?? process.cwd(),
      tsConfigFilePath: opts.tsconfig ?? fileConfig.tsConfigFilePath ?? "tsconfig.json",
      include: opts.include ?? fileConfig.include ?? [],
      exclude: opts.exclude ?? fileConfig.exclude ?? [],
    };

    const server = createRetangleServer(Number(opts.port));
    server.start();
